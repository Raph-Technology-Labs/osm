...
def defect_totals(db: Session, session_id: int) -> dict:
    """Per-defect-class counts.

    `deciding` counts the label that actually made a camera NOK -- that is the
    number an operator acts on. `detected` counts every class the model
    reported, including ones under an allowed_defects filter that were
    therefore ignored. Reporting only the second would overstate failures;
    reporting only the first hides a model that is seeing things.
    """
