...
# --- which card does a row belong to? ---------------------------------------
#
# Three authorities, in descending order of trust:
#
# 1. THE CONFIG (camera_pipelines, built by the router's _camera_pipelines
#    from the resolved machine config). machine_config.yaml states it exactly:
#    s1's measurement.allowed_cameras is [cam1], s2's defect.allowed_cameras
#    is [cam2]. This never guesses, so it wins whenever it has an answer.
#
# 2. pipeline_name on the row -- authoritative the day CameraResultItem writes
#    a real value. Today it is hardcoded "pipeline1", which is why it can't be
#    relied on alone yet.
#
# 3. The payload itself, as a last resort for a config that failed to resolve
#    or a caller that passes no map.
#
# Why any of this matters: CameraResult.is_defective is set for every failing
# camera regardless of pipeline -- inspection_session computes
# `passed = not captured.is_defect` for measurement cameras too, so an
# out-of-tolerance part sets it exactly like a real defect does. Counting it
# blind put measurement failures in the Defect card. An earlier fix -- "skip
# any row with measurement_data" -- overcorrected: a camera running BOTH
# pipelines vanished from the Defect card entirely.
...
def _configured_pipeline(cam: CameraResult, camera_pipelines: dict[str, str]) -> str | None:
    """Authority 1 -- what the machine config says this camera runs."""
    return camera_pipelines.get(cam.camera_id)


def _is_defect_row(cam: CameraResult, camera_pipelines: dict[str, str] | None = None) -> bool:
    configured = _configured_pipeline(cam, camera_pipelines or {})
    if configured:
        return configured == "defect"
...
def _is_measurement_row(cam: CameraResult, camera_pipelines: dict[str, str] | None = None) -> bool:
    configured = _configured_pipeline(cam, camera_pipelines or {})
    if configured:
        return configured == "measurement"

    declared = _declared_pipeline(cam)
    if declared:
        return declared == "measurement"

    # Payload-driven, as before: a row with no measurement_data contributes
    # nothing here whatever it claims to be.
    return _has_measurement(cam)


def defect_totals(
    db: Session,
    session_id: int,
    camera_pipelines: dict[str, str] | None = None,
) -> dict:
    """Per-defect-class counts, over DEFECT rows only.
    ...
    for cam, _ in _camera_results(db, session_id):
        if not _is_defect_row(cam, camera_pipelines):
            continue
...
def measurement_totals(
    ...
    for cam, _ in _camera_results(db, session_id):
        if not _is_measurement_row(cam, camera_pipelines):
            continue
        for name, m in (cam.measurement_data or {}).items():
            if not isinstance(m, dict):
                continue
...
def recent_events(
    ...
    for r in rows:
        ...
        for cam in r.camera_results:
            # Same classification the cards use, so the log and the cards can
            # never label the same row differently.
            if _is_measurement_row(cam, camera_pipelines) and _has_measurement(cam):
                pipeline = "measurement"
                detail = ", ".join(
                    f"{name} {m.get('measured')}{m.get('unit') or ''}"
                    for name, m in cam.measurement_data.items()
...
def build_analysis(
    ...
    cams = camera_pipelines or {}
    kinds = set(cams.values())
    return {
        "session_id": session_id,
        # What this machine is actually configured to do, so the UI can show a
        # card only for a pipeline that exists -- an empty Defect card on a
        # measurement-only machine reads as a broken detector rather than as
        # "there is no detector here". Both true when no map was supplied:
        # showing everything beats hiding real results.
        "pipelines": {
            "defect": "defect" in kinds if kinds else True,
            "measurement": "measurement" in kinds if kinds else True,
        },
        "defects": defect_totals(db, session_id, cams),
        "measurements": measurement_totals(db, session_id, cams),
        "stations": station_totals(db, session_id),
    }
...
def report_rows(
    ...
    for r in fires:
        ...
        for cam in r.camera_results:
            ...
            if measurements and _is_measurement_row(cam, camera_pipelines):
                # One row per parameter. A part measured on diameter AND
                # length produces two rows, so neither value has to share a
                # column with the other.
                for name, m in measurements.items():
